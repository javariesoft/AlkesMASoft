...
private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
    try {
        ...
        String[] kode = com.erv.function.Util.split(akt.getACCAKUMULASIPENYUSUTAN(), ".");
        String[] kode1 = com.erv.function.Util.split(akt.getACCBIAYAPENYUSUTAN(), ".");
        p = dbpr.getDetails(c, kode[0]);
        cboKelPerkiraan1.setSelectedItem(p.getKODEPERKIRAAN() + "-" + p.getNAMAPERKIRAAN());
        p = dbpr.getDetails(c, kode1[0]);
        cboKelPerkiraan2.setSelectedItem(p.getKODEPERKIRAAN() + "-" + p.getNAMAPERKIRAAN());
...
// TODO add your handling code here:
    try {
        txtAkumulasiPenyusutan.setText(AktivatetapDao.setKodeAktivaTetap(c,akunperkiraan1[cboKelPerkiraan1.getSelectedIndex()]));
    } catch (Exception e) {
    }
...
// TODO add your handling code here:
    try {
        txtACCBiayaPenyusutan.setText(AktivatetapDao.setKodeAktivaTetap(c,akunperkiraan2[cboKelPerkiraan2.getSelectedIndex()]));
    } catch (Exception e) {
    }
    ...
    void isiCombo() {
        ...
        try {
            ...
            ps = con.prepareStatement("select * from PERKIRAAN where TIPE='D' and GRUP='1' AND (left(KODEPERKIRAAN,5)='12200' or left(KODEPERKIRAAN,5)='12300' or left(KODEPERKIRAAN,5)='12400')", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = ps.executeQuery();
            rs.last();
            if (rs.getRow() > 0) {
            ...
            ps = con.prepareStatement("select * from PERKIRAAN where TIPE='D' and GRUP='1' AND (left(KODEPERKIRAAN,5)='12201' or left(KODEPERKIRAAN,5)='12301' or left(KODEPERKIRAAN,5)='12401')", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = ps.executeQuery();
            rs.last();
            if (rs.getRow() > 0) {
            ...
            ps = con.prepareStatement("select * from PERKIRAAN where TIPE='D' and kodeperkiraan='61201'", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = ps.executeQuery();
            rs.last();
            if (rs.getRow() > 0) {
                akunperkiraan2 = new String[rs.getRow()];
                rs.beforeFirst();
                int c = 0;
                cboKelPerkiraan2.removeAllItems();
                while (rs.next()) {
                    cboKelPerkiraan2.addItem(rs.getString(1) + "-" + rs.getString(2));
                    akunperkiraan2[c] = rs.getString(1);
                    c++;
                }
            }
        } catch (Exception ex) {
