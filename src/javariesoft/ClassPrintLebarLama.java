 ...
 * @author UserXP
 */
public class ClassPrintLebarLama {
        Connection c;
    public static void main(String[] args) {
        //Connection c;       
        ClassPrintLebarLama cs = new ClassPrintLebarLama();
        try { 
    ...
    public void cetakfaktur(String nfak){
        ...
        ClassPrintLebarLama cp = new ClassPrintLebarLama();
        ...
        try {
            ...
            try {
                ...
                p.append("---------------------------------------------------------------------------------------------------------------------------------------------------- \r\n");
                p.append("| Kepada : "+ cp.cetak(pel, 136, 0) +"|\r\n");
                p.append("|       "+cp.cetak(Altpel, 139, 0) +"|\r\n");
                p.append("---------------------------------------------------------------------------------------------------------------------------------------------------- \r\n");
                ...
                Logger.getLogger(ClassPrintLebarLama.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            while (rs.next()) {
                ...
                try {
                    p.append(cp.cetak("|", 1, 1) + ""+ cp.cetak(""+no, 3, 1) + " | " + cp.cetak(df.format(Integer.parseInt(rs.getString(12)))/* jmlh */, 6, 1) + " | " + cp.cetak(rs.getString(40) /*satuan*/ , 15, 0) + " | " + cp.cetak(nmbrg, 44, 0) + " | " + cp.cetak(rs.getString(41) /*Batch*/ , 10, 0) + " |" + cp.cetak(rs.getString(42) /*Expire*/ , 10, 0) + " | " + cp.cetak(df.format(rs.getDouble(13))/* harga */, 13, 1) + " | " + cp.cetak(df.format(rs.getDouble(43))/* Disc */, 5, 1) + " | " + cp.cetak(df.format(rs.getDouble(15))/* total */, 16, 1) + " |\r\n");
                } catch (UnsupportedEncodingException ex) {
                    Logger.getLogger(ClassPrintLebarLama.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            ...
            try {
                ...
                p.append(cp.cetak("| ", 108, 0)+" Diskon Tambahan : |"+cp.cetak(df.format(rs1.getDouble(9)), 18, 1)+" |\r\n");
                p.append(cp.cetak("| ", 120, 0)+" PPN : |"+cp.cetak(df.format(rs1.getDouble(44)), 18, 1)+" |\r\n");
                //p.append(cp.cetak("| Terbilang : " + com.erv.fungsi.Fungsi.bilang((long) total), 103, 0)+"     Bayar : |"+cp.cetak(df.format(rs1.getDouble(8)), 18, 1)+" |\r\n");
                p.append(cp.cetak("| Terbilang : " + com.erv.fungsi.Fungsi.bilangDouble((double) terbilang), 114, 0)+"     Bayar : |"+cp.cetak(df.format(rs1.getDouble(8)), 18, 1)+" |\r\n");
                p.append(cp.cetak("| ", 110, 0)+"         Total : |"+cp.cetak(df.format(totalbersih), 18, 1)+" |\r\n");
                p.append("----------------------------------------------------------------------------------------------------------------------------------------------------\r\n");
                p.append(cp.cetak(" ", 120, 0)+" Padang, "+cp.cetak(d.format(tgl), 18, 0)+"\r\n");
